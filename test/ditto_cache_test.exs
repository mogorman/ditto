defmodule Ditto.CacheTest do
  ...
  setup do
    ...
    ExUnit.Callbacks.on_exit(fn -> Ditto.Cache.invalidate() end)

    :ok
  end

  test "get_or_run" do
    assert 10 == Ditto.Cache.get_or_run(__MODULE__, :func, [], fn -> 10 end)
    assert 10 == Ditto.Cache.get_or_run(__MODULE__, :func, [], fn -> 10 end)
    assert 1 == Ditto.Cache.invalidate(__MODULE__, :func, [])
    assert 0 == Ditto.Cache.invalidate(__MODULE__, :func, [])
    assert 10 == Ditto.Cache.get_or_run(__MODULE__, :func, [], fn -> 10 end)
  end
  ...
  test "many processes call get_or_run, but one process only calls the caching function" do
    fun = fn ->
      assert 20 == Ditto.Cache.get_or_run(__MODULE__, :key, [], fn -> cache_single(100, 20) end)
    end
  ...
  test "if caching process is crashed when waiting many processes, one of the processes call the caching function" do
    fun1 = fn ->
      assert_raise(RuntimeError, "failed", fn ->
        Ditto.Cache.get_or_run(__MODULE__, :key, [], fn -> cache_raise(100, 20) end)
      end)
    end

    fun2 = fn ->
      assert 30 == Ditto.Cache.get_or_run(__MODULE__, :key, [], fn -> cache_raise(100, 30) end)
    end
  ...
  test "after garbage_collect/0 is called, expired value is collected" do
    ...
    assert 10 ==
             Ditto.Cache.get_or_run(__MODULE__, :key3, [], fn ->
    ...
    assert 1 == Ditto.Cache.garbage_collect()
  ...
  test "if the memory usage exceeds the max_threshold, unused cached values is evicted" do
    opts = Application.fetch_env!(:ditto, Ditto.CacheStrategy.Eviction)
  ...
  test "if :permanent is specified in get_or_cache's opts, the cached value won't be collected by garbage_collect/0" do
    ...
    Ditto.Cache.get_or_run(__MODULE__, :key, [], fn -> 10 end, permanent: true)
    eat_memory(min_threshold)
    Ditto.Cache.garbage_collect()

    # :key's value is not collected
    assert 10 == Ditto.Cache.get_or_run(__MODULE__, :key, [], fn -> 20 end, permanent: true)
  end
  ...
  test "after expires_in milliseconds have elapsed, all expired value is collected" do
    ...
    assert 20 ==
             Ditto.Cache.get_or_run(__MODULE__, :key, [], fn ->
               cache_with_call_count(:key, 0)
             end)

    assert 10 ==
             Ditto.Cache.get_or_run(__MODULE__, :key2, [], fn ->
